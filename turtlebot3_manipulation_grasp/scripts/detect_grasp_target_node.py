...
class DetectGraspTargetNode(Node):
    ...
    def __init__(self, pipeline_provider=None, **node_kwargs):
        # node_kwargs 原样转给 rclpy.Node（patrol 托管时要用它传
        # parameter_overrides=[Parameter("use_sim_time", ...)] —— 本节点必须
        # 跟随 Gazebo 的 /clock，否则给抓取侧的时间戳会被判"时钟域不一致"✗）。
        # 单独运行时不传，行为与原来一致。
        super().__init__("detect_grasp_target", **node_kwargs)

        # ── 共享流水线模式（patrol_task 同进程托管本节点时用）──────
        # 调用方给一个零参 callable，返回**它自己已经建好的** VisionPipeline。
        # 给了就不再自己建模型 —— 两个进程各一套 GDINO+SAM2+INSID3 会双份占显存，
        # 合起来 ~5 GiB/8.15 GiB → 双双 CUDA OOM ✗
        # 为 None 时（独立运行 / --self-test / dining_grasp_task 调试）行为与原来完全一致。
        ...
        # 共享模式下模型本来就在调用方进程里，自己再预热一遍纯属白占显存 ✗
    ...
    def _load_pipeline_unlocked(self):
        # 共享模式下调用方（patrol_task）的脚本目录本来就在 sys.path 上，
        # 不依赖这边的探测结果，找不到也不必提前退出 ✗
        if self._nav2_scripts is None and self._pipeline_provider is None:
            self._vp_error = "找不到 vision_pipeline.py（先 source 工作区 install/setup.bash）"
            return False
        if self._nav2_scripts is not None and self._nav2_scripts not in sys.path:
        ...
        try:
            if self._pipeline_provider is None:
                ...
                self.get_logger().info("首次调用：向调用方索取共享视觉流水线...")
            ...
            try:
                ...
                self.get_logger().warn("insid3_review 不可用（{}）→ 抓取模式只能用自由文本标签"
                                       .format(e))
            if self._pipeline_provider is not None:
                # ★ 共享模式：拿调用方（patrol_task）已建好的那套，绝不自己再造 ✗
                #   provider 内部一般就是 patrol 的 _ensure_pipeline()：拿它的锁、等它预热完，
                #   返回的就是同一个对象 → 全系统只有一份模型常驻。
                try:
                    self._vp = self._pipeline_provider()
                except Exception as e:                   # noqa: BLE001
                    self._vp_error = "共享流水线获取失败：{}: {}".format(type(e).__name__, e)
                    self.get_logger().error(self._vp_error)
                    return False
                if self._vp is None:
                    self._vp_error = "调用方未能提供共享视觉流水线（patrol 侧加载失败？）"
                    self.get_logger().error(self._vp_error)
                    return False
                self.get_logger().info("视觉流水线就绪（复用调用方进程内的共享模型，耗时 {:.1f}s）；词表 {}"
                                       .format(time.time() - t0, vp.ITEM_NAMES))
                return True
            ...
            if self._pipeline_provider is not None:
                # 共享模式：模型在调用方进程里，这里失败跟 venv 无关，别给错提示 ✗
                self.get_logger().error(
                    "共享视觉流水线不可用：{}\n"
                    "  （模型本应由调用方 patrol_task 加载；查它那边的加载日志）✗"
                    .format(self._vp_error))
                return False
            self.get_logger().error(
                "视觉流水线加载失败：{}\n"
                "  多半是没在视觉虚拟环境里跑（torch/groundingdino/sam2 不在系统 python 里）✗\n"
                "  先：source ~/vision_env/bin/activate".format(self._vp_error))
            return False
