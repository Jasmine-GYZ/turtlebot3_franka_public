...
private:
  ...
  // ★ 只规划、不执行（调试开关，默认 false = 正常抓取）。
  //   为什么需要：MTC 的 plan() 能单独暴露 INIT_FAILED(4) / NO_SOLUTION(5)，
  //   而这两个失败与"机械臂真的动起来"无关。开着它就能在不冒"撞翻桌上其它物品 /
  //   要人工收拾现场"风险的前提下反复试 grasp_lift、grasp_frame_transform 这些参数。
  //   注意：stage 仍返回 OK（success=true），因为"规划成功"确实成立了 ——
  //   但它**不代表抓到了东西**，message 里会写明未执行 ✗
  ...
  void loadParams() {
    ...
    plan_only_ = paramOr<bool>("plan_only", plan_only_);
    if (plan_only_) {
      RCLCPP_WARN(LOGGER, "★ plan_only=true：只规划、不执行，机械臂不会动（调试模式）");
    }
...
GraspResult MTCTaskNode::run(const GraspRequest& request) {
  ...
  // 5) execute（plan_only 时跳过：只验证规划侧，机械臂不动）
  if (plan_only_) {
    RCLCPP_WARN(LOGGER, "plan_only=true → 跳过执行（上面这份解只用于验证规划，机械臂没动）");
    result.success = true;
    result.stage = GraspStage::OK;
    result.message = "plan_only：规划成功，但【未执行】（机械臂没动，也没抓到东西）";
    return result;
  }
