...
class CompetitionTask(Node):
    ...
    def provide_pipeline(self):
        """给同进程的抓取服务节点（DetectGraspTargetNode）当 pipeline_provider。

        ★ 存在的意义就是**共享同一套模型**：抓取服务不再自己 VisionPipeline()，
        于是 GDINO+SAM2+INSID3 全系统只加载一份（原来两份合占 ~5 GiB/8.15 GiB，
        双双 CUDA OOM → Phase 2 直接报废 ✗）。
        加载失败返回 None（不是 False！服务侧只判 `is None`）。
        """
...
def main():
    ...
    # ── Phase 2 的视觉服务：★ 同进程托管，复用本节点已加载的那套模型 ────
    #   原来靠 grasp_service.launch.py 另起一个进程，两边各加载一整套
    #   GDINO+SAM2+INSID3 → 合计 ~5 GiB / 8.15 GiB → 双双 CUDA OOM，
    #   Phase 2 直接报废 ✗（2026-09-16 实测）。
    #   现在把 DetectGraspTargetNode 挂进本进程的 executor，它通过
    #   pipeline_provider 拿到 node._pipeline 的**同一个对象** → 显存只占一份 ✓
    #   服务名 /detect_grasp_target 不变，grasp_phase.py 一行都不用改。
    vision_node = None
    if node.run_grasp:
        try:
            from ament_index_python.packages import get_package_prefix
            grasp_lib = os.path.join(
                get_package_prefix("turtlebot3_manipulation_grasp"),
                "lib", "turtlebot3_manipulation_grasp")
            if grasp_lib not in sys.path:
                sys.path.insert(0, grasp_lib)
            from detect_grasp_target_node import DetectGraspTargetNode
            vision_node = DetectGraspTargetNode(
                pipeline_provider=node.provide_pipeline,
                parameter_overrides=[Parameter("use_sim_time", value=bool(USE_SIM_TIME))],
            )
            print("=== grasp 视觉服务已在本进程托管（共享同一套模型）===", flush=True)
        except Exception as e:                    # noqa: BLE001
            # 服务起不来只影响 Phase 2，基础题照跑，不许把整场带崩 ✗
            vision_node = None
            print("=== ⚠️ 进程内抓取视觉服务启动失败（Phase 2 会拿不到目标）: {} ==="
                  .format(e), flush=True)

    # start() 内部的导航结果通过 action callback 异步处理。
    # ★ 必须多线程 executor：Phase 2 的抓取阶段在"轮询等服务/等 future"，
    #   单线程下会把回调堵死 ✗（视觉适配层那边已经踩过一次同样的坑）
    # ★ 现在这个 executor 托管**两个**节点（patrol + 抓取视觉服务），而服务回调
    #   本身很长（多帧投票 = 跑好几遍 detect，最长十几秒），跑起来时相机订阅、
    #   导航 action 反馈都还得有线程可用 → 线程数从 3 提到 5，留足余量。
    from rclpy.executors import MultiThreadedExecutor
    ex = MultiThreadedExecutor(num_threads=5)
    ex.add_node(node)
    if vision_node is not None:
        ex.add_node(vision_node)
    try:
        ex.spin()
